  ...
  class SitemapGenerator < Generator
    ...
    def generate_content(site)
      ...
      site.pages.each do |page|
        ...
        mod_date = (File.exist?(site.source + path) ? File.mtime(site.source + path) : Time.now)
        ...
        next if path =~ %r!.(sass|scss|css|yml)$!

        # Remove the trailing 'index.html' if there is one, and just output the folder name.
        path = path[0..-12] if path =~ /\/index.html$/

        changefreq = if page.data.key?("changefreq")
                       page.data["changefreq"]
                     else
                       ""
                     end

        result += entry(path, mod_date, changefreq, site) unless path =~ %r!(error)|(feed)|(quiz)!
      end

      # Next, find all the posts.
      posts = site.site_payload["site"]["posts"]
      for post in posts do
        changefreq = if post.data.key?("changefreq")
