...
if ($("#stream1").length > 0) {
  ...
  function stream_page(page_active) {
    ...
    for (var i = 2; i <= stream_data_rows; i++) {
      if (typeof (stream_data[i]['A']) != "undefined") sa = stream_data[i]['A'];
      else continue;
    ...
    for (var i = 2; i <= stream_data_rows; i++) {
      ...
      if (typeof (stream_data[i]['B']) != "undefined") sb = stream_data[i]['B'];
      else sb = '';
      if (typeof (stream_data[i]['C']) != "undefined") sc = stream_data[i]['C'];
      else sc = '';
      if (typeof (stream_data[i]['D']) != "undefined") sd = stream_data[i]['D'];
      else sd = '';
      if (typeof (stream_data[i]['E']) != "undefined") se = stream_data[i]['E'];
      else se = '';
  ...
  function set_video() {
    ...
    if (web_stream_status == new_stream_status) {
      return;
    }
